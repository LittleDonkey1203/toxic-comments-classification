...
# def k_split(data_len, k):
#     data_range = range(data_len)
#     data_idx = random.sample(data_range, data_len)
#     data_num_1 = np.ceil(data_len / k)
#     data_num_2 = np.floor(data_len / k)
#     data_more_num = data_len % k
#     list_begin = 0
#     list_all = []
#     for ii in range(k):
#         if ii < data_more_num:
#             list_tmp = data_idx[list_begin:list_begin + data_num_1]
#             list_begin += data_num_1
#         else:
#             list_tmp = data_idx[list_begin:list_begin+data_num_2]
#             list_begin += data_num_2
...
if __name__ == '__main__':
    ...
    list_sentences_train, list_sentences_test, y_train =\
        word_parse.get_train_test(True)    # Get train and test input and train target
