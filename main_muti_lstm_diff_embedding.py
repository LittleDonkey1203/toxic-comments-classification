...
from model import nlp_model, multi_nlp_model
...
if __name__ == '__main__':
    ...
    EMBEDDING_TYPE = [EMBEDDING_TYPE_glove, EMBEDDING_TYPE_fasttext]
    ...
    word_parse = WordParse(TRAIN_DATA_FILE, TEST_DATA_FILE, MAX_NB_WORDS, MAX_SEQUENCE_LENGTH, EMBEDDING_DIM)
    ...
    embedding_matrix_glove = word_parse.get_embedding_matrix(
        [embeddings_index_glove],
        [EMBEDDING_DIM],
        [EMBEDDING_TYPE_glove])  # Get embedding matrix by embedding index, this step has to be after encoded the all scentences, and has word_list information

    embedding_matrix_fasttext = word_parse.get_embedding_matrix(
        [embeddings_index_fasttext],
        [EMBEDDING_DIM],
        [EMBEDDING_TYPE_fasttext])
    ...
    nlp_model = \
        multi_nlp_model(BATCH_SIZE_TRAIN, BATCH_SIZE_TEST, EPOCHS,
                  EMBEDDING_DIM, OUTPUT_SIZE, OUTPUT_FEATURE_NUM, DENSE_HIDDEN_NUM, MAX_SEQUENCE_LENGTH,
    ...
    for train_idx, val_idx in kf.split(list_sentences_train):
        ...
        nlp_model.get_bidirectional_multi_lstm_att(embedding_matrix, str(EMBEDDING_TYPE) + '_' + str(bagging_iter))   # Get lstm attention model
